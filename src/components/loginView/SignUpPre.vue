  ...
  <div>
    ...
    <div class="content_table" v-if="editFlag">
      <ul>
        ...
        <li class="ul_li_dub"><span>卖家账号:</span><b style="display: inline-block; width: 200px">{{userName}}</b><span>店铺名称:</span><input v-model="company.adShopName" type="text"/></li>
...
//        alert(test)
      ...
      subExInfo () {
        ...
        if (companyPhonef && companyCodef && companyMailf && companyContactsf2 && bankNumberf2 && shopPhonef2) {
          this.$store.dispatch('upShop', this.company).then(
            ...
            this.$message.error('注册失败，请检查参数，稍后再试')
          })
        }
      }
    },
    created () {
      ...
      console.log(parse)
      this.companyName = parse.companyName
      this.userName = parse.userName
      this.company.adId = parse.adId
    }
  }
</script>
<style>
